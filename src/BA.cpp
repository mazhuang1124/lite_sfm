        ...
        for (auto &observedFrame:mapPointPos.first->observedFrames) {
            ceres::CostFunction *costFunction =
                    new ceres::AutoDiffCostFunction<ReprojectCost, 2, 4, 6, 3>(
                            new ReprojectCost(observedFrame.second));   // ==> cv::Point2d observation(2);
            problem.AddResidualBlock(
                    costFunction,                                       // calc cost
                    lossFunction,
                    cameraIntrinsics[observedFrame.first->camera].val,  // Intrinsic(4)
                    frameExtrinsics[observedFrame.first].val,           // Extrinsic(6)
                    mapPointPos.second.val                              // ==> cv::Matx13d  pos3d(3)
            );
        }
    }
